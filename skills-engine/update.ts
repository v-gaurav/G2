...
import { BASE_DIR, G2_DIR } from './constants.js';
  ...
  try {
    ...
    for (const relPath of preview.filesChanged) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `g2-update-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
    ...
    const remapFile = path.join(newCorePath, '.g2-meta', 'path_remap.yaml');
