...
import { BASE_DIR, CUSTOM_DIR, G2_DIR } from './constants.js';
import { initG2Dir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initG2Dir();
  console.log('Skills system initialized. .g2/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .g2/custom/migration.patch',
      );
    } else {
