...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'g2-test-'));
}

export function setupG2Dir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.g2', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.g2', 'backup'), { recursive: true });
}

export function writeState(tmpDir: string, state: any): void {
  const statePath = path.join(tmpDir, '.g2', 'state.yaml');
