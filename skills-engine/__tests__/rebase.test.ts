...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.g2', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.g2', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.g2', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.g2', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.g2', 'base');
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.g2', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.g2', 'base', 'src', 'config.ts'),
      'utf-8',
    );
  ...
  it('rebase clears resolution cache', async () => {
    ...
    const resDir = path.join(tmpDir, '.g2', 'resolutions', 'skill-a+skill-b');
    ...
    const resolutions = fs.readdirSync(
      path.join(tmpDir, '.g2', 'resolutions'),
    );
