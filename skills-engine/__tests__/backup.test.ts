...
import { createTempDir, setupG2Dir, cleanup } from './test-helpers.js';
...
describe('backup', () => {
  ...
  it('clearBackup removes backup directory', () => {
    ...
    const backupDir = path.join(tmpDir, '.g2', 'backup');
  ...
  it('createBackup writes tombstone for non-existent files', () => {
    ...
    const tombstone = path.join(tmpDir, '.g2', 'backup', 'src', 'newfile.ts.tombstone');
