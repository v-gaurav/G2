...
function buildContainerArgs(
  ...
  runtime: IContainerRuntime = defaultRuntime,
): string[] {
  const args: string[] = ['run', '-i', '--rm', '--name', containerName];

  // Pass host timezone so the container agent knows the correct local date/time.
  // TIMEZONE is already validated in config.ts; guaranteed to be a valid IANA string or 'UTC'.
  ...
  for (const mount of mounts) {
    if (mount.readonly) {
      args.push(...runtime.readonlyMountArgs(mount.hostPath, mount.containerPath));
    } else {
      args.push(...runtime.readwriteMountArgs(mount.hostPath, mount.containerPath));
    }
  }
...
export async function runContainerAgent(
  ...
  return new Promise((resolve) => {
    const container = spawn(runtime.bin, containerArgs, {
    ...
    const killOnTimeout = () => {
      ...
      exec(runtime.stopContainer(containerName), { timeout: 15000 }, (err) => {
        if (err) {
