...
vi.mock('../infrastructure/Config.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../infrastructure/Config.js')>();
  return {
    ...actual,
    ASSISTANT_NAME: 'G2',
    TRIGGER_PATTERN: /^@G2\b/i,
  };
});

import { TRIGGER_PATTERN } from '../infrastructure/Config.js';
import {
  escapeXml,
  formatMessages,
  formatOutbound,
...
describe('TRIGGER_PATTERN', () => {
  it('matches @G2 at start of message', () => {
    expect(TRIGGER_PATTERN.test('@G2 hello')).toBe(true);
  });
  ...
  it('does not match when not at start of message', () => {
    expect(TRIGGER_PATTERN.test('hello @G2')).toBe(false);
  });
  ...
  it('matches with word boundary before apostrophe', () => {
    expect(TRIGGER_PATTERN.test("@G2's thing")).toBe(true);
  });

  it('matches @G2 alone (end of string is a word boundary)', () => {
    expect(TRIGGER_PATTERN.test('@G2')).toBe(true);
  });

  it('matches with leading whitespace after trim', () => {
    // The actual usage trims before testing: TRIGGER_PATTERN.test(m.content.trim())
    expect(TRIGGER_PATTERN.test('@G2 hey'.trim())).toBe(true);
  });
});
...
describe('trigger gating (requiresTrigger interaction)', () => {
  ...
  it('non-main group with requiresTrigger=true processes when trigger present', () => {
    const msgs = [makeMsg({ content: '@G2 do something' })];
