...
import { MAX_CONCURRENT_CONTAINERS } from '../infrastructure/Config.js';
import { IpcTransport } from '../ipc/IpcTransport.js';
...
export class GroupQueue {
  ...
  enqueueTask(groupJid: string, taskId: string, fn: () => Promise<void>): void {
    ...
    if (state.active) {
      ...
      // Close the idle interactive container so the task can run once it exits
      this.closeStdin(groupJid);
      logger.debug({ groupJid, taskId }, 'Container active, task queued — closing idle container');
      return;
    }
  ...
  sendMessage(groupJid: string, text: string): boolean {
    ...
    return this.transport.sendMessage(state.groupFolder, text);
  }
  ...
  closeStdin(groupJid: string): void {
    ...
    this.transport.closeStdin(state.groupFolder);
  }
