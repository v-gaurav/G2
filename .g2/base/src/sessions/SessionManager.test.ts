...
describe('loadFromDb', () => {
  it('loads sessions from DB into memory', () => {
    database.sessionRepo.setSession('group-a', 'sess-1');
    database.sessionRepo.setSession('group-b', 'sess-2');
...
describe('set', () => {
  it('sets session in memory and DB', () => {
    ...
    expect(sm.get('group-a')).toBe('sess-1');
    // DB
    expect(database.sessionRepo.getSession('group-a')).toBe('sess-1');
  });
  ...
  it('overwrites existing session', () => {
    ...
    expect(database.sessionRepo.getSession('group-a')).toBe('sess-2');
  });
});
...
describe('delete', () => {
  it('removes session from memory and DB', () => {
    ...
    expect(database.sessionRepo.getSession('group-a')).toBeUndefined();
  });
