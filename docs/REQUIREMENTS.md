...
- **Authorization as a policy class** — `authorization.ts` exports an `AuthorizationPolicy` class that encapsulates checks for a single `AuthContext`, plus backward-compatible standalone guard functions (`canSendMessage`, `canScheduleTask`, `canRegisterGroup`, etc.). No side effects, trivially testable.
- **Single responsibility modules** — Each concern has its own module: trigger validation (`trigger-validator`), mount security (`mount-security`), timeout configuration (`TimeoutConfig` in `config`), secure env parsing (`env`), outbound message rate limiting (`outgoing-message-queue`), message formatting (`message-formatter`), message routing (`message-router`), path construction (`group-paths`), retry logic (`retry`), safe JSON parsing (`safe-parse`).
