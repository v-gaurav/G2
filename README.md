...
<p align="center">
  ...
  <!-- token-count --><a href="https://github.com/v-gaurav/g2/tree/main/repo-tokens">55.4k tokens · 28% of context window</a><!-- /token-count -->
</p>
...
3. `npm run build`
4. `./container/build.sh` to build the agent container image
5. `npm start` (or `npm run dev` for development)
6. Scan the QR code with WhatsApp to link the device
7. Send yourself a message: `@G2 hello` to register the main channel
...
| `src/app.ts` | Composition root: wires all services |
| `src/messaging/MessagePoller.ts` | Message polling, cursor management, trigger checking |
| `src/execution/AgentExecutor.ts` | Container execution, session tracking |
| `src/execution/ContainerRunner.ts` | Spawns agent containers |
| `src/execution/ExecutionQueue.ts` | Per-group queue with global concurrency limit |
| `src/messaging/ChannelRegistry.ts` | Multi-channel registry with prefix routing |
| `src/scheduling/TaskScheduler.ts` | Runs scheduled tasks when due |
| `src/sessions/SessionManager.ts` | Session resume and archive management |
| `src/ipc/IpcWatcher.ts` | Host-side IPC file watcher |
| `src/groups/Authorization.ts` | Fine-grained IPC authorization |
| `src/execution/MountSecurity.ts` | Mount allowlist validation |
| `groups/*/CLAUDE.md` | Per-group persistent memory |

## Configuration

Configuration lives in `src/infrastructure/Config.ts`. Key environment variables:
...
- **Non-root** — container runs as unprivileged `node` user (uid 1000)
...
| Container exit code 1 | Check logs; verify mount is `/home/node/.claude/` not `/root/.claude/` |
