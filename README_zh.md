...
G2 用一个你能在 8 分钟内理解的代码库，为你提供了同样的核心功能。只有一个进程，寥寥数个文件。智能体（Agent）运行在具有文件系统隔离的真实 Linux 容器中，而不是依赖于权限检查。

## 快速上手

```bash
git clone https://github.com/gavrielc/g2.git
cd g2
claude
```
...
使用触发词（默认为 `@G2`）与你的助手对话：

```
@G2 每周一到周五早上9点，给我发一份销售渠道的概览（需要访问我的 Obsidian vault 文件夹）
@G2 每周五回顾过去一周的 git 历史，如果与 README 有出入，就更新它
@G2 每周一早上8点，从 Hacker News 和 TechCrunch 收集关于 AI 发展的资讯，然后发给我一份简报
```
...
如果你想添加 Telegram 支持，不要创建一个 PR 同时添加 Telegram 和 WhatsApp。而是贡献一个技能文件 (`.claude/skills/add-telegram/SKILL.md`)，教 Claude Code 如何改造一个 G2 安装以使用 Telegram。
