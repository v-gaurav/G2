...
|-----|----------|---------|
| **Main app logs** | `logs/g2.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/g2.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash g2-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash g2-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash g2-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash g2-agent:latest
```
...
# Also clear the session ID from G2's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/g2.log | tail -5
...
```bash
echo "=== Checking G2 Container Setup ==="
...
SESSIONS=$(grep "Session initialized" logs/g2.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
