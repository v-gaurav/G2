...
vi.mock('grammy', () => ({
  Bot: class MockBot {
    ...
    start(opts: { onStart: (botInfo: any) => void }) {
      opts.onStart({ username: 'g2_ai_bot', id: 12345 });
    }
...
import { TelegramChannel, TelegramChannelOpts } from './TelegramChannel.js';
...
describe('TelegramChannel', () => {
  ...
  describe('@mention translation', () => {
    it('translates @bot_username mention to trigger format', async () => {
      ...
      const ctx = createTextCtx({
        text: '@g2_ai_bot what time is it?',
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@G2 @g2_ai_bot what time is it?',
        }),
      );
    });
    ...
    it('does not translate if message already matches trigger', async () => {
      ...
      const ctx = createTextCtx({
        text: '@G2 @g2_ai_bot hello',
        entities: [{ type: 'mention', offset: 6, length: 12 }],
      });
      await triggerTextMessage(ctx);

      // Should NOT double-prepend — already starts with @G2
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@G2 @g2_ai_bot hello',
        }),
      );
    });
    ...
    it('handles mention in middle of message', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@G2 hey @g2_ai_bot check this',
        }),
      );
    });
  ...
  describe('bot commands', () => {
    ...
    it('/ping replies with bot status', async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith('G2 is online.');
    });
  });
