---
name: setup
description: Run initial G2 setup. Use when user wants to install dependencies, authenticate WhatsApp, register their main channel, or start the background services. Triggers on "setup", "install", "configure g2", or first-time setup requests.
---
...
AskUserQuestion: What trigger word? (default: G2). In group chats, messages starting with @TriggerWord go to Claude. In the main channel, no prefix needed.
...
5. Pick the most likely candidates (e.g. groups with the trigger word or "G2" in the name, small/solo groups) and present them as AskUserQuestion options — show names only, not JIDs. Include an "Other" option if their group isn't listed. If they pick Other, search by name in the DB or re-run with a higher limit.
...
- `--assistant-name "Name"` — if trigger word differs from "G2"

## 9. Mount Allowlist

AskUserQuestion: Want the agent to access directories outside the G2 project? (Git repos, project folders, documents, etc.)
...
If the service is already running (check `launchctl list | grep g2` on macOS), unload it first: `launchctl unload ~/Library/LaunchAgents/com.g2.plist` — then proceed with a clean install.
...
- On macOS: check `launchctl list | grep g2` to see if it's loaded with an error status. If the PID column is `-` and the status column is non-zero, the service is crashing. Read `logs/g2.error.log` for the crash reason and fix it (common: wrong Node path, missing .env, missing auth).
- On Linux: check `systemctl --user status g2` for the error and fix accordingly.
...
- SERVICE=stopped → run `npm run build` first, then restart: `launchctl kickstart -k gui/$(id -u)/com.g2` (macOS) or `systemctl --user restart g2` (Linux). Re-check.
...
Show the log tail command: `tail -f logs/g2.log`

## Troubleshooting

**Service not starting:** Check `logs/g2.error.log`. Common causes: wrong Node path in plist (re-run step 10), missing `.env` (re-run step 4), missing WhatsApp auth (re-run step 5).
...
**WhatsApp disconnected:** Run `npm run auth` to re-authenticate, then `npm run build && launchctl kickstart -k gui/$(id -u)/com.g2`.

**Unload service:** `launchctl unload ~/Library/LaunchAgents/com.g2.plist`
