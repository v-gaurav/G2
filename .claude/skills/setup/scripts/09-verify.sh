...
if [ "$PLATFORM" = "macos" ]; then
  if launchctl list 2>/dev/null | grep -q "com.g2"; then
    ...
    LAUNCHCTL_LINE=$(launchctl list 2>/dev/null | grep "com.g2" || true)
  ...
  if systemctl --user is-active g2 >/dev/null 2>&1; then
    SERVICE="running"
  elif systemctl --user list-unit-files 2>/dev/null | grep -q "g2"; then
    SERVICE="stopped"
  fi
fi
log "Service: $SERVICE"
...
if [ -f "$PROJECT_ROOT/.env" ]; then
  if grep -qE "^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|CLAUDE_CODE_USE_BEDROCK)=" "$PROJECT_ROOT/.env" 2>/dev/null; then
...
if [ -f "$PROJECT_ROOT/store/messages.db" ]; then
  REGISTERED_GROUPS=$(node --no-warnings -e 'const db=require("better-sqlite3")("./store/messages.db");try{console.log(db.prepare("SELECT COUNT(*) as cnt FROM registered_groups").get().cnt)}catch{console.log(0)}' 2>/dev/null || echo "0")
fi
log "Registered groups: $REGISTERED_GROUPS"

# 6. Check mount allowlist
MOUNT_ALLOWLIST="missing"
if [ -f "$HOME/.config/g2/mount-allowlist.json" ]; then
