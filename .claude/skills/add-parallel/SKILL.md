...
Adds Parallel AI MCP integration to G2 for advanced web research capabilities.
...
- **Non-blocking Design** - Uses G2 scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. G2 already set up and running
...
Add Parallel AI MCP servers after the g2 server:
```typescript
const mcpServers: Record<string, any> = {
  g2: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__g2__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__g2__send_message
   4. Use mcp__g2__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo g2-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/g2.log | grep "scheduled task"`
