---
name: add-gmail
description: Add Gmail integration to G2. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail capabilities to G2. It can be configured in two modes:
...
> How do you want to use Gmail with G2?
>
> **Option 1: Tool Mode**
> - Agent can read and send emails when you ask it to
> - Triggered only from WhatsApp (e.g., "@G2 check my email" or "@G2 send an email to...")
...
>    - If prompted for consent screen, choose "External", fill in app name (e.g., "G2"), your email, and save
>    - For Application type, select **Desktop app**
>    - Name it anything (e.g., "G2 Gmail")
>    - Click **Create**
...
```bash
sleep 2 && launchctl list | grep g2
```
...
>
> `@G2 check my recent emails`
>
> Or:
>
> `@G2 list my Gmail labels`
...
Complete all Tool Mode steps above before continuing. Verify Gmail tools work by having the user test `@G2 check my recent emails`.
...
export const EMAIL_CHANNEL: EmailChannelConfig = {
  ...
  triggerValue: 'G2',  // the label name, address pattern, or prefix
  contextMode: 'thread',
...
```bash
sleep 3 && tail -20 logs/g2.log | grep -i email
```
...
```bash
tail -f logs/g2.log | grep -E "(email|Email)"
```
