...
This skill adds Discord support to G2 using the skills engine for deterministic code changes, then walks through interactive setup.

## Phase 1: Pre-flight

### Check if already applied

Read `.g2/state.yaml`. If `discord` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
> 2. Click **New Application** and give it a name (e.g., "G2 Assistant")
...
- @mention translation (Discord `<@botId>` → G2 trigger format)
