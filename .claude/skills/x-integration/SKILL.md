---
name: x-integration
description: X (Twitter) integration for G2. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** G2 v1.0.0. Directory structure may change in future versions.
...
1. **G2 is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep g2 shows PID and exit code 0
```
...
| `G2_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/g2.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into G2, make the following modifications:
...
All paths below are relative to project root (`G2_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep g2  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/g2.log | tail -20

# Script errors
grep -i "error\|failed" logs/g2.log | tail -20
```
...
docker run g2-agent ls -la /app/src/skills/
```
